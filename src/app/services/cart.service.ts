  ...
  AddProductToCart(id: Number, quantity?: number) {
    ...
    this.productService.getSingleProduct(id).subscribe(prod => {
      ...
      if (this.cartDataServer.data[0].product === undefined) {
        ...
        localStorage.setItem('cart', JSON.stringify(this.cartDataClient));
        this.cartDataObs$.next({...this.cartDataServer});
        this.toast.success(`${prod.name} added to the cart.`, "Đã thêm sản phẩm", {
          timeOut: 1500,
          progressBar: true,
      ...
      // Cart is not empty
        ...
        if (index !== -1) {
          ...
          this.toast.info(`${prod.name} số lượng đã được cập nhật trong giỏ hàng.`, "Đã cập nhật Giỏ hàng", {
            timeOut: 1500,
            progressBar: true,
            progressAnimation: 'increasing',
            positionClass: 'toast-top-right'
        ...
        // 2. If chosen product is not in cart array
          ...
          this.toast.success(`${prod.name} thêm vào giỏ hàng.`, "Đã thêm vào Giỏ hàng", {
            timeOut: 1500,
            progressBar: true,
            progressAnimation: 'increasing',
            positionClass: 'toast-top-right'
